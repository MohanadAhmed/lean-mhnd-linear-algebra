...
begin
  ...
  {  simp_rw [(spectralAHA), Vblock, Sblock, reindex_apply, ← submatrix_map],
     rw [conj_transpose_submatrix,  from_blocks_conj_transpose, hS₁₂, hS₂₁, hS₂₂,
      from_blocks_map, ← submatrix_mul, ← submatrix_mul],
     simp_rw [(matrix.map_zero _ (map_zero RηC)), from_blocks_multiply, matrix.mul_zero, 
      add_zero, matrix.zero_mul, add_zero, matrix.mul_empty, matrix.empty_mul],
     apply submatrix_empty_blocks,
