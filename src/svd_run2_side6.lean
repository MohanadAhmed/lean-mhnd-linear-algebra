...
begin
  ...
  rw ← submatrix_one_equiv eb.symm,
  rw ← from_blocks_one,
  congr,
  
  funext x y,
  rw dmatrix.add_apply,
  rw mul_apply,
  rw mul_apply,
  simp_rw [of_apply, conj_transpose_apply, of_apply],
  simp_rw fintype.sum_subtype_add_sum_subtype pn (λ g, V x g * star (V y g)),
  conv_lhs {apply_congr, skip, rw ← conj_transpose_apply, },
  rw ← mul_apply,  rw mul_eq_one_comm at Vinv, rw Vinv, 
